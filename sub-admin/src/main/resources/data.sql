...
INSERT INTO customers (id, uuid, first_name, last_name, email, phone_number, created_at, updated_at) VALUES
                                                                                                         (0, gen_random_uuid(), 'Walk-in', 'Customer', 'walkin@example.com', '123-456-7890', NOW(), NOW()),
...
-- 4. Products
-- Link products to users and categories
INSERT INTO products (id, name, description, is_active, category_id, created_by_user_id, created_at, updated_at) VALUES
                                                                                                                               (1, 'iPhone 15 Pro', 'The latest and greatest iPhone.', true, (SELECT id FROM categories WHERE name = 'Smartphones'), 2, NOW(), NOW()),
                                                                                                                               (2, 'Galaxy S24 Ultra', 'Samsung''s flagship with AI features.', true, (SELECT id FROM categories WHERE name = 'Smartphones'), 2, NOW(), NOW()),
                                                                                                                               (3, 'MacBook Pro 16"', 'Powerful laptop for professionals.', true, (SELECT id FROM categories WHERE name = 'Laptops'), 2, NOW(), NOW()),
                                                                                                                               (4, 'MX Keys S', 'Advanced wireless illuminated keyboard.', true, (SELECT id FROM categories WHERE name = 'Keyboards'), 3, NOW(), NOW()),
                                                                                                                               (5, 'WH-1000XM5', 'Industry-leading noise canceling headphones.', true, (SELECT id FROM categories WHERE name = 'Headphones'), 3, NOW(), NOW()),
                                                                                                                               (101, 'Google Pixel 8 Pro', 'Google''s flagship phone with advanced AI.', true, (SELECT id FROM categories WHERE name = 'Smartphones'), 2, NOW(), NOW()),
                                                                                                                               (102, 'Dell XPS 15', 'High-performance laptop for creative professionals.', true, (SELECT id FROM categories WHERE name = 'Laptops'), 3, NOW(), NOW());
...
INSERT INTO product_variants (id, product_id, sku, price, cost_price, weight, attributes, created_at, updated_at) VALUES
                                                                                                                      ...
                                                                                                                      (101, 101, 'GGL-PX8P-128-BLK', 799.00, 550.00, 0.213, '{"color": "Obsidian", "storage": "128GB"}', NOW(), NOW()),
                                                                                                                      (102, 102, 'DEL-XPS15-512-SLV', 1899.00, 1300.00, 1.92, '{"color": "Platinum Silver", "storage": "512GB"}', NOW(), NOW());
...
INSERT INTO orders (id, uuid, customer_id, status, total_amount, billing_address, shipping_address, order_date, created_at, updated_at) VALUES
                                                                                                                                      (1, gen_random_uuid(), 1, 'DELIVERED', 1108.99, '123 Main St, Anytown, USA', '123 Main St, Anytown, USA', NOW() - INTERVAL '10 day', NOW() - INTERVAL '10 day', NOW() - INTERVAL '5 day'),
                                                                                                                                      (2, gen_random_uuid(), 2, 'SHIPPED', 1299.00, '456 Oak Ave, Somecity, USA', '456 Oak Ave, Somecity, USA', NOW() - INTERVAL '3 day', NOW() - INTERVAL '3 day', NOW() - INTERVAL '1 day'),
                                                                                                                                      (3, gen_random_uuid(), 1, 'PENDING', 399.99, '123 Main St, Anytown, USA', '789 Pine Ln, Otherville, USA', NOW(), NOW(), NOW());

-- 8. Order Items
-- Link order items to orders and product variants
-- Order 1
INSERT INTO order_items (id, uuid, order_id, product_variant_id, quantity, price_per_unit, line_total) VALUES
                                                                                                     (1, gen_random_uuid(), 1, 1, 1, 999.00, 999.00),
                                                                                                     (2, gen_random_uuid(), 1, 5, 1, 109.99, 109.99);

-- Order 2
INSERT INTO order_items (id, uuid, order_id, product_variant_id, quantity, price_per_unit, line_total) VALUES
    (3, gen_random_uuid(), 2, 3, 1, 1299.00, 1299.00);

-- Order 3
INSERT INTO order_items (id, uuid, order_id, product_variant_id, quantity, price_per_unit, line_total) VALUES
    (4, gen_random_uuid(), 3, 6, 1, 399.99, 399.99);

-- 9. Payments
-- Link payments to orders
INSERT INTO payments (id, uuid, order_id, amount, currency, status, payment_method, metadata, created_at, updated_at) VALUES
                                                                                                                    (1, gen_random_uuid(), 1, 1108.99, 'USD', 'COMPLETED', 'CARD', '{"transaction_id": "ch_123abc"}', NOW() - INTERVAL '10 day', NOW() - INTERVAL '10 day'),
                                                                                                                    (2, gen_random_uuid(), 2, 1299.00, 'USD', 'COMPLETED', 'CARD', '{"transaction_id": "ch_456def"}', NOW() - INTERVAL '3 day', NOW() - INTERVAL '3 day'),
                                                                                                                    (3, gen_random_uuid(), 3, 399.99, 'USD', 'PENDING', 'CASH', NULL, NOW(), NOW());
